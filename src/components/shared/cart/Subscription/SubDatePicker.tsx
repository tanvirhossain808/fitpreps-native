...
export default function SubDatePicker({
  cartType = 'meals',
  handleDateSelect,
  date = null,
  isDisabled = false,
        ...
        <YStack gap={12} px="$4" py="$5">
          ...
          <TouchableOpacity
            ...
            <Text alignSelf="stretch" flex={1} color="#8E95A2" fontSize={14}>
              {date?.dateString ? date?.dateString : 'Pick delivery date'}
            </Text>
            {showDatePicker ? (
        ...
        {showDatePicker && (
          <Portal>
            <View
              ...
              right={0}
              bottom={0}>
              <DatePicker toggleDatePicker={toggleDatePicker} date={date} setDate={setDate} />
            </View>
          </Portal>
        )}
      </YStack>

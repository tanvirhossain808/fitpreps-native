...
const useProductFilters = (initialProducts: Productsmakelijke[] = [], productType: string) => {
  ...
  const productsByCategory = useMemo(() => {
    ...
    return productType === 'fueld'
      ? initialProducts.filter((p) => p.categories[0]?.toLowerCase() === category?.toLowerCase())
      : initialProducts.filter((p) => p.categories[1]?.toLowerCase() === category?.toLowerCase());
  }, [initialProducts, category, productType]);
